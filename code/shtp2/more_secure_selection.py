...
# more secure selection protocol k = 1, 2, 3, 4, 5
...
def load_data(filename, k):
    ...
    crx = encrypt(pub, rx)
    print "Random number= ", rx
    dist = {}
    for i in range(nrows):
        dist[i + 1] = int(decrypt(priv, pub, long(table.cell_value(i, 0)) * crx))
    print "SSED= ", dist
    if k == 1:
        result = more_secure_selection_1(dist)
    elif k == 2:
        result = more_secure_selection_2(dist)
    elif k == 3:
        result = more_secure_selection_3(dist)
    elif k == 4:
        result = more_secure_selection_4(dist)
    elif k == 5:
        result = more_secure_selection_5(dist)
    else:
        print "Wrong Input!!"
    print "SSO1NN= ", result


def more_secure_selection_1(x):
...
def more_secure_selection_2(x):
    ss = sorted(x.items(), key=lambda d: d[1])
    print "After sorting dist= ", ss
    return ss[0], ss[1]


def more_secure_selection_3(x):
    ss = sorted(x.items(), key=lambda d: d[1])
    print "After sorting dist= ", ss
    return ss[0], ss[1], ss[2]


def more_secure_selection_4(x):
    ss = sorted(x.items(), key=lambda d: d[1])
    print "After sorting dist= ", ss
    return ss[0], ss[1], ss[2], ss[3]


def more_secure_selection_5(x):
    ss = sorted(x.items(), key=lambda d: d[1])
    print "After sorting dist= ", ss
